...
class AcceptReferralUseCase(private val repository: ReferralRepository) {
    suspend operator fun invoke(referralId: String): Resource<Referral?> {
        val result = repository.acceptReferral(referralId)
        val response = result.data ?: return Resource.Error(result.message ?: "Failed to accept referral")
